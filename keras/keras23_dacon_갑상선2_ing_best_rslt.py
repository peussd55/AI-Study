...
# SMOTE 데이터 증폭(테스트데이터는 하면안된다.), DNN은 SMOTE하면 성능 더 안좋음
# 증폭 전 데이터 확인
...
# 2. 모델구성 (Dense)
model = Sequential()
model.add(Dense(256, input_dim=feature_dim, activation='relu', kernel_regularizer=l2(0.001)))   # activation 없앨 것
...
"""
model = load_model('C:\\Study25\\_save\\dacon\\갑상선\\k23_0624_1536_0142-0.4862_Dense.hdf5', custom_objects={'F1Score': F1Score})
model.summary()
# Dense(units=64, input_dim=14, name='dense')
# BatchNormalization(name='batch_normalization'),
# Dropout(rate=0.5, name='dropout')     # 정확한 rate 모름
# Dense(units=32, name='dense_1'),
# Dense(units=16, name='dense_2'),
# Dense(units=1, activation='sigmoid', name='dense_3') # 이진 분류의 경우
...
for thresh in thresholds:
    ...
    # 2. 계산된 F1 Score를 set에 추가합니다. (중복 값은 자동으로 무시됩니다)
    unique_f1_scores.add(current_f1)
...
# f1 = f1_score(y_test, y_pred)
# 3. 루프가 끝난 후, set을 리스트로 변환하여 출력합니다. 가독성을 위해 정렬할 수 있습니다.
final_unique_f1_list = sorted(list(unique_f1_scores), reverse=True)
print("\n=== 중복되지 않는 F1 Score 목록 (내림차순) ===")
print(final_unique_f1_list)
# [0.49949545913218973, 0.49924357034795763, 0.49899193548387094, 0.498989898989899, 
# 0.4987405541561713, 0.49873801110550225, 0.4984894259818731, 0.49823855057876193, 
# 0.4982314300151592, 0.49798792756539234, 0.497737556561086, 0.4977238239757208, 
# 0.49748743718592964, 0.4974721941354904, 0.4972375690607735, 0.49698795180722893, 
# 0.49696356275303644, 0.4969574036511156, 0.4967385850476668, 0.496705524581855, 
# 0.49648946840521563, 0.49645390070921985, 0.49624060150375937, 0.4962025316455696, 
# 0.4961948249619482, 0.49599198396793587, 0.214300348289285]
