...
from tensorflow.keras.layers import Dense, SimpleRNN, LSTM, GRU, Dropout, BatchNormalization, Conv1D, Flatten, MaxPooling1D, Reshape
...
data = split_xy(dataset=xy[:-timesteps], timesteps=timesteps, step=36)
print(data)
...
x_train, x_test, y_train, y_test = train_test_split(
    x, y, test_size=0.2, shuffle=False  # 시간순서 유지하기 위해 shuffle은 False
)
# train, val 분할 (9:2)
x_train, x_val, y_train, y_val = train_test_split(
    x_train, y_train, test_size=2/9, shuffle=False  # 시간순서 유지하기 위해 shuffle은 False
)
...
# 2. 모델구성
# model = Sequential()
# # model.add(LSTM(64, input_shape=(144, 11), activation='tanh', return_sequences=True))    # return_sequences=True : 3차원(batch, timesteps, feature) 출력 유지
# # 일반적으로 시계열 레이어는 다단(2층)이상 쌓는다고 성능이 좋아지진 않는다. : 첫번째 시계열레이어의 출력이 시계열형태라고 보장할 수없기때문
# # 반면 CNN은 다단으로 쌓을 수록 성능이 좋아진다.    
# model.add(Conv1D(filters=512, kernel_size=2, padding='same', input_shape=(144,11)))  # (None, 144, 256)
# model.add(Conv1D(filters=256, kernel_size=2, padding='same'))  # (None, 144, 128)
# model.add(Conv1D(filters=128, kernel_size=2, padding='same'))   # (None, 144, 64)
...
# # 1번
# model = Sequential()
# # 입력 레이어: kernel_size를 늘려 한 번에 더 넓은 시간 범위를 보도록 설정
# # padding='causal'은 현재 타임스텝 예측 시 미래 데이터를 보지 않도록 방지합니다.
# model.add(Conv1D(filters=64, kernel_size=7, padding='causal', activation='relu', input_shape=(144, 11)))
# model.add(BatchNormalization())
# model.add(Dropout(0.2))
# # 중간 레이어: Dilation_rate를 사용해 수용 영역을 효율적으로 확장
# model.add(Conv1D(filters=128, kernel_size=7, padding='causal', activation='relu', dilation_rate=2))
# model.add(BatchNormalization())
# model.add(Dropout(0.2))
# # 특징을 종합하기 위해 Flatten 레이어 사용
# model.add(Flatten())
# # 종합된 특징으로 예측 수행
# model.add(Dense(128, activation='relu'))
# model.add(Dense(144, activation='linear')) # 출력층: 144개 타임스텝을 직접 예측
# # 최종 출력 형태를 y_train의 shape (None, 144)와 맞추기 위한 Reshape
# # Keras의 loss 함수는 보통 (None, 144, 1)과 (None, 144)를 자동으로 처리하지만, 명시적으로 맞춰주는 것이 안전합니다.
# model.add(Reshape((144, 1)))
# model.summary()

# 2번
model = Sequential()
# 입력 레이어 및 중간 레이어 (제안 1과 유사)
# Dilation을 통해 효율적으로 장기 의존성 학습
model.add(Conv1D(filters=64, kernel_size=7, padding='causal', activation='relu', input_shape=(144, 11)))
model.add(BatchNormalization())
model.add(Conv1D(filters=128, kernel_size=7, padding='causal', activation='relu', dilation_rate=2))
model.add(BatchNormalization())
model.add(Conv1D(filters=256, kernel_size=7, padding='causal', activation='relu', dilation_rate=4))
model.add(BatchNormalization())
model.add(Dropout(0.3))
# 출력 레이어: kernel_size=1 Conv1D를 사용하여 각 타임스텝의 피처(64개)를 1개의 예측값으로 압축
# 이 레이어는 시퀀스 길이를 유지하면서 채널(피처) 수만 1로 줄입니다.
...
print('RMSE :', rmse)
# 걸린시간: 88.3062493801117 초
# RMSE : 67.5912526735243

# 걸린시간: 208.04694294929504 초
# RMSE : 69.00357402820477

# 걸린시간: 292.66431856155396 초
# RMSE : 61.175778533995064

# 걸린시간: 603.6256048679352 초
# RMSE : 56.10961962737971

# 걸린시간: 1244.2447571754456 초
# RMSE : 55.27607778746795

# 걸린시간: 482.86299753189087 초
# RMSE : 62.34346438747822
