...
# C:\Study25\_data\tensor_cert\horse-or-human
...
print(x_train.shape, x_test.shape)  # (718, 200, 200, 3) (309, 200, 200, 3)
...
augment_size = 300      # 300개 증폭시킬것임
print(x_train.shape)    # (718, 200, 200, 3)
print(x_train.shape[0]) # 718
...
print(np.min(randidx), np.max(randidx))     # 0 717

# 데이터생성
x_augmented = x_train[randidx].copy()   # 300개의 데이터 copy, copy로 새로운 메모리 할당. 서로영향X
y_augmented = y_train[randidx].copy()
print(x_augmented.shape)    # (300, 200, 200, 3)
print(x_augmented.shape)    # (300, 200, 200, 3)

# # 차원변환 : 이미 4차원이라 변환 불필요
# # x_augmented = x_augmented.reshape(300, 200, 200, 3)
...
x_augmented = datagen.flow(
    ...
    shuffle=False,
    save_to_dir='c:\\study25\\_data\\_save_img\\06_horse\\',  # 증폭데이터 저장경로
).next()[0] #튜플에서 x만 추출
exit()
...
print(x_train.shape, y_train.shape) # (1018, 200, 200, 3) (1018,)
...
# 걸린시간 : 128.41 초
# acc(sklearn 지표) : 0.9773462783171522
# 증폭하니까 성능 떨어짐
