...
x = train_csv.drop(['count'], axis=1)    # test셋에는 없는 casual, registered, y가 될 count는 x에서 제거
...
random_state 2222222222 // 32-16-8-4-2-1 // 1000 //32
loss 값 : 22738.369140625
r2 스코어: 0.3435059418912739
rmse 스코어: 150.79246652345557
"""
...
submission_csv.to_csv(f'{path}new_submission22_{current_time}.csv', index=False)

"""
# to_csv 함수
index=True (default)
인덱스 없는 데이터프레임을 to_csv 할 경우 : 디폴트 인덱스 자동생성
인덱스 있는 데이터프레임을 to_csv 할 경우 : 디폴트 인덱스 자동생성안함

index=False
인덱스 없는 데이터프레임을 to_csv 할 경우 : 디폴트 인덱스 자동생성안함
인덱스 있는 데이터프레임을 to_csv 할 경우 : 원래 인덱스 삭제함

# read_csv 함수
디폴트 인덱스를 자동으로 생성함. csv파일 자체에서 인덱스를 구분하는 정보가 없기때문에 자동생성하는듯함.
따라서 인덱스로 지정할 컬럼옵션(index_col)을 줘야한다.
