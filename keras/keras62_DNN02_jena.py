...
"""
## 56-jena 카피
## DNN 방식2 : 기존 LSTM방식에서 DNN방식으로 변경(데이터구조 split를 바꾸고, reshape는 그대로두기)
...
def split_xy(dataset, timesteps, step):
    ...
    print('total_len:', total_len)  # 420551
    # 144스텝으로 나누어떨어지기위해 마지막 288개는 남기고, 그 위 71개는 건너뜀
    exclude_start = total_len - timesteps* - 71
    exclude_end = total_len - timesteps
    print('exclude_start:', exclude_start)  # 430775
    print('exclude_end:', exclude_end)      # 420407
...
data = split_xy(dataset=xy[:], timesteps=timesteps, step=72)
print(data)
print(data.shape)   # (2920, 144, 12)

# x 생성
x = data[:-1, :, :-1]  # x의 마지막 샘플 제거(마지막은 predict으로 들어가야함)하고 마지막 열(y)제거
print(x)
print(x.shape)      # (2919, 144, 11)
...
x_train, x_test, y_train, y_test = train_test_split(
    x, y, test_size=0.2, shuffle=False  # Dense이므로 shuffle 사용해도무방
)
# train, val 분할 (9:2)
x_train, x_val, y_train, y_val = train_test_split(
    x_train, y_train, test_size=2/9, shuffle=False  # Dense이므로 shuffle 사용해도무방
)

print("x_train shape:", x_train.shape)  # (3630, 144, 11)
print("x_val shape:", x_val.shape)      # (1038, 144, 11)
print("x_test shape:", x_test.shape)    # (1167, 144, 11)
print("y_train shape:", y_train.shape)  # (3630, 144)
print("y_val shape:", y_val.shape)      # (1038, 144)
...
# 걸린시간: 134.03988885879517 초
# RMSE : 61.697615866690306

# -> DNN1방식과 마찬가지로 오래 학습할수록 val_loss와 지표와 상관없이 성능 떨어짐. 속도는 훨씬 빠름
