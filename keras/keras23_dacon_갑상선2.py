...
###################################################################################################
# 상관관계 분석을 통한 불필요한 컬럼 식별
import seaborn as sns
import matplotlib.pyplot as plt

# 상관관계 히트맵 생성
plt.figure(figsize=(12, 10))
correlation_matrix = train_csv.corr()
sns.heatmap(correlation_matrix, annot=True, cmap='coolwarm', center=0)
plt.title('Feature Correlation Matrix')
# plt.show()

# 타겟과 상관관계가 낮은 컬럼 제거 (절댓값 0.05 미만)
target_corr = correlation_matrix['Cancer'].abs().sort_values(ascending=False)
print("타겟과의 상관관계:")
print(target_corr)

# 상관관계가 낮은 컬럼 제거
low_corr_cols = target_corr[target_corr < 0.05].index.tolist()
if 'Cancer' in low_corr_cols:
    low_corr_cols.remove('Cancer')

print(f"제거할 컬럼들: {low_corr_cols}")
# 제거할 컬럼들: ['TSH_Result', 'T3_Result', 'T4_Result', 'Nodule_Size', 'Age']
train_csv = train_csv.drop(columns=low_corr_cols)
test_csv = test_csv.drop(columns=low_corr_cols)
...
# 2. 모델구성 (Dense)
model = Sequential()
model.add(Dense(64, input_dim=9, activation='relu'))   # activation 없앨 것
...
thresholds = np.arange(0.25, 0.86, 0.0001)
best_f1 = 0
best_threshold = 0.5
