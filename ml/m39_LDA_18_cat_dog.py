...
# cnn으로 학습된걸 => 평탄화 -> lda -> dnn 과정으로 변경
# 45카피
...
# 이미 정규화한 데이터를 .numpy로 저장하고 불러오기때문에 정규화필요X
...
# LDA 적용
lda = LinearDiscriminantAnalysis(n_components=1)   # LDA의 n_components는 최대 클래스(y)의 수-1개 까지만 적용가능
...
# [LDA 적용]
# loss : 1.4748988151550293
# acc : 0.5522666573524475
...
# ##### csv 파일 만들기 (lda로 차원변환해줘서 csv파일만들때도 submission_x 변환해야함)#####
