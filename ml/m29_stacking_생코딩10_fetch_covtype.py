...
for i, model in enumerate(models):
    ...
    for train_idx, val_idx in kf.split(x):
        ...
        meta_features[val_idx, i] = val_pred.ravel()  # 1차원으로 변환 추가
        
        # CV 성능 확인
        cv_score = accuracy_score(y_val, val_pred.ravel())  # 여기도 수정
...
# 7. 메타 모델 학습
meta_model = meta_model = LogisticRegression(
    random_state=42, 
    max_iter=1000,
    C=0.1  # 강한 정규화
)
...
# XGBClassifier: 0.871423
# RandomForestClassifier: 0.955371
# CatBoostClassifier: 0.885476
# LGBMClassifier: 0.854806
# Cross-Validation 스태킹: 0.936490
