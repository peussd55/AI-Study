...
# best_r2_score : 0.8504349409098124

print('걸린시간 :', round(end - start), '초')
# 걸린시간 : 30 초
# -> 교차검증횟수가 줄어서 속도도 확연히 빨라진다.
...
pd.DataFrame(model.cv_results_).sort_values('rank_test_score', ascending=True).to_csv(path + 'm18_ rs_cv_results_02_california.csv')

# 5. 가중치 저장
path = './_save/m18_cv_results/'
joblib.dump(model.best_estimator_, path + 'm18_best_model_02_california.joblib')
