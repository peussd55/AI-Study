...
class RNN(nn.Module):
    ...
    def forward(self, x):
        ...
        ##### 2. view : 에러 (view는 연속적 메모리 데이터에만 해당) // 연속적메모리 : 데이터가 순차적으로 저장되어있지않은 상태로 바뀌었음
        # x = x.view(-1, 3*32)
        
        ##### 3. contiguous를 이용해서 연속적인 메모리라고 인식시킨다. (view 쓰는이유? reshape보다 속도에서 이점이 있음)
        x = x.contiguous()
        x = x.view(-1, 3*32)
